...
install_conda_and_create_env() {
    ...
    # If the "cs3244_proj" environment exists, update it; otherwise, create it.
    if conda env list | grep -q "^cs3244_proj"; then
        echo "Conda environment 'cs3244_proj' already exists. Updating it..."
        conda activate cs3244_proj
        conda env update -f environment.yaml 
    else
        echo "Creating Conda environment 'cs3244_proj'..."
        conda create --name cs3244_proj python=3.9 -y
        conda activate cs3244_proj
...
ipython kernel install --user --name=cs3244_proj --display-name "(cs3244_proj)"